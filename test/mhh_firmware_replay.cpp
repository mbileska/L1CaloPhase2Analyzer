...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "Usage: mhh_firmware_replay INPUT OUTPUT\n";
    return 2;
  }
  ...
  while (std::getline(input, line)) {
    ...
    for (unsigned int link = 0; link < kInputLinks; ++link) {
      if (!(parser >> token)) {
        std::cerr << "Input row has fewer than 16 links\n";
        return 2;
      }
  ...
  for (unsigned int event = 0; event < rows.size() / kWordsPerLink; ++event) {
    ap_uint<576> linkIn[kInputLinks] = {};
    ap_uint<576> linkOut[kOutputLinks] = {};
    for (unsigned int word = 0; word < kWordsPerLink; ++word) {
      for (unsigned int link = 0; link < kInputLinks; ++link) {
        linkIn[link].range(word * 64 + 63, word * 64) = rows[event * kWordsPerLink + word][link];
      }
    }
    mhh_algo_top(linkIn, linkOut);
    for (unsigned int word = 0; word < kWordsPerLink; ++word) {
      output << std::setw(4) << std::setfill('0') << std::hex << word;
      for (unsigned int link = 0; link < kOutputLinks; ++link) {
        output << "    " << std::setw(16) << std::setfill('0') << std::nouppercase
               << linkOut[link].range(word * 64 + 63, word * 64).to_uint64();
      }
      output << '\n';
    }
  }
